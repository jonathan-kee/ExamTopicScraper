...
# Answers
A DDL statement issued by a session with an uncommitted transaction automatically commits that transaction. D.

A. DML statements always start new transactions.

An uncommitted transaction is automatically committed when the user exits SQL*Plus. E.

A session can see uncommitted updates made by the same user in a different session. C.
