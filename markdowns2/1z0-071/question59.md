...
# Answers
A role is a named group of related privileges that can only be assigned to a user. C.

A. Roles are assigned to users using the ALTER USER statement.

A single role can be assigned to multiple users. G.

A single user can be assigned multiple roles. D.
