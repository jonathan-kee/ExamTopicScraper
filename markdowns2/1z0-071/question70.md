...
# Answers
A FOREIGN KEY column in a child table and the referenced PRIMARY KEY column in the parent table must have the same names. C.

A. NOT NULL can be specified at the column and at the table level.

A table can have multiple PRIMARY KEY and multiple FOREIGN KEY constraints. E.

A table can have only one PRIMARY KEY and one FOREIGN KEY constraint. B.

A table can have only one PRIMARY KEY but may have multiple FOREIGN KEY constraints. F.
